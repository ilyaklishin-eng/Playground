(() => {
  ...
  const buildPhonePayload = (node) => {
    const textNode = node.querySelector(".obfuscated-phone");
    const obfuscated = (textNode?.textContent || "").replace(/\s+/g, " ").trim();
    if (!obfuscated) return null;

    const visible = Array.from(obfuscated).reverse().join("");
    const tel = visible.replace(/[^\d+]/g, "");
    if (!tel) return null;

    return { visible, tel };
  };

  const attachPhoneLink = (node) => {
    if (!node || node.dataset.phoneBound === "1") return;
    const payload = buildPhonePayload(node);
    if (!payload) return;

    node.dataset.phoneBound = "1";
    const activate = () => {
      if (node.dataset.phoneReady === "1") return;
      node.dataset.phoneReady = "1";
      node.setAttribute("href", `tel:${payload.tel}`);
      node.setAttribute("aria-label", `Call ${payload.visible}`);
      node.setAttribute("title", payload.visible);
    };

    node.addEventListener("mouseenter", activate, { passive: true });
  ...
  const init = () => {
    ...
    document.querySelectorAll("a.js-phone").forEach(attachPhoneLink);
  };
